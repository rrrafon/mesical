...
def mixAllfade(dir = DIR,   ext = '.mp3', fileName = 'mixedOutput'):
    ...
    for i in range(lenNotes):
        chords += '-i %s%s '%(notes[i][0], ext)
        delays += '[%s]adelay=%s|delay%s[chord%s];' % (str(i+1), musicTime, str(i+1), str(i+1))
        mix += '[chord%s]' % str(i+1)
        musicTime += int(WHOLENOTE/int(notes[i][1]))
    cmd =  'ffmpeg %s -filter_complex %s [0]%s amix=%s %s%s' % (chords, delays, mix, lenNotes+1, fileName, ext)
    print(cmd)
